...
# Lo usamos para importar `NetworkModel` exclusivamente para anotaciones
...
class BaseAgent(mesa.Agent):  # type: ignore
    """
    Agente abstracto. No implementa comportamiento: solo guarda el `node_id`
    y delega `step()` en las subclases.

    Atributos añadidos sobre `mesa.Agent`:
      - `node_id` (int): id del nodo del grafo donde "vive" este agente.
    """
    ...
    def __init__(self, model: "NetworkModel", node_id: int) -> None:
        ...
        # Guardamos la posición del agente en el grafo
