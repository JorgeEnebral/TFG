...
# AGENTE
# ============================================================
class MessageAgent(mesa.Agent):
    """Cerebro mínimo: con probabilidad p, dispara un mensaje a un vecino."""

    def __init__(self, model, node_id: int, fire_probability: float):
...
class NetworkModel(mesa.Model):
    ...
    def __init__(self, G: nx.Graph, fire_probability: float = 0.15, seed: int = 42):
        ...
        for node_id in G.nodes():
            agent = MessageAgent(self, node_id=node_id,
...
def run_simulation(
    ...
    def draw_frame(frame: int):
        ...
        nx.draw_networkx_edges(
            G, pos, ax=ax,
            edge_color="#2a3142", width=1.0, alpha=0.5,
        )
        ...
        if model.active_messages:
            ...
            nx.draw_networkx_edges(
                ...
                edge_color="#ff6b9d", width=3.0, alpha=0.95,
...
def parse_args():
    ...
    p.add_argument("--edge-prob", type=float, default=0.25, help="Probabilidad de arista (Erdős–Rényi)")
    p.add_argument("--fire-prob", type=float, default=0.20, help="Probabilidad de disparo por paso")
    p.add_argument("--time", type=int, default=40, help="Duración (pasos) de la simulación")
    p.add_argument("--interval", type=int, default=500, help="Intervalo entre frames en ms")
    p.add_argument("--seed", type=int, default=42, help="Semilla")
    p.add_argument("--no-gif", action="store_true", help="No guardar GIF")
    p.add_argument("--show", action="store_true", help="Mostrar ventana en vivo (en local)")
    p.add_argument("--out", type=str, default="simulation.gif", help="Ruta del GIF de salida")
    return p.parse_args()


if __name__ == "__main__":
    args = parse_args()
    _, model = run_simulation(
    ...
    print(f"[OK] Total mensajes disparados: {len(model.message_history)}")
